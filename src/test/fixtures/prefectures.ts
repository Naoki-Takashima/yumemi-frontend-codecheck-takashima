/**
 * 都道府県一覧のテスト用データ。
 * 実 API（GET /api/v1/prefectures）のレスポンスをそのまま写している。
 */
export const prefecturesFixture = [
...
/** prefCode から都道府県名を引く。フィクスチャに無い場合は例外を投げる。 */
export function prefNameOf(prefCode: number): string {
  const found = prefecturesFixture.find((p) => p.prefCode === prefCode);
  if (!found) {
    throw new Error(`フィクスチャに prefCode=${prefCode} が存在しません`);
  }
  return found.prefName;
}
